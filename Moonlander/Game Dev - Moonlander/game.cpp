...
{
   drawText(Point(30,160), "It's a lander! Level 1");
   if (lander.isAlive() && !lander.isLanded())
   {
      ...
      if (ui.isDown())
      {
         ...
         drawText(Point(20,20), "Going Up!");
      }
      
      if (ui.isLeft())
      {
         lander.applyThrustLeft();
         drawText(Point(20,40), "Going Right!");
      }
      
      if (ui.isRight())      {
         lander.applyThrustRight();
         drawText(Point(20,60), "Going Left!");
      }
   }
}
...
void Game :: draw(const Interface & ui)
{
      ...
      {
         fuelLocation.setX(topLeft.getX() + 5);
         fuelLocation.setY(topLeft.getY() - 30);
         drawText(fuelLocation, "Low fuel");
      }
